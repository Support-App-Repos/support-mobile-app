/**
 * My Listings Screen — Figma My Listings (1055:771)
 */
...
import { useMyListingActions, useWishlist } from '../hooks';
  ...
  const fetchListings = useCallback(async () => {
    try {
      ...
      if (response.success && Array.isArray(listingsData)) {
        setListings(dedupeListingsById(listingsData));
      }
    } catch (error: any) {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <BottomNavigation
        ...
        onCreatePress={() => navigation?.navigate('SelectCategory')}
        showCreateButton
      />
